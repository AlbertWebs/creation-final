<?php

// Suppress PHP 8.4 deprecation warnings from vendor packages
// E_STRICT is deprecated in PHP 8.4, so we only suppress E_DEPRECATED
error_reporting(E_ALL & ~E_DEPRECATED);

// Set error handler to filter deprecation warnings BEFORE autoloader
set_error_handler(function ($errno, $errstr, $errfile, $errline) {
    // Suppress deprecation warnings
    if ($errno === E_DEPRECATED) {
        return true; // Suppress the warning
    }
    // Let other errors through to default handler
    return false;
}, E_ALL & ~E_DEPRECATED);

use Illuminate\Contracts\Http\Kernel;
use Illuminate\Http\Request;

define('LARAVEL_START', microtime(true));

/*
|--------------------------------------------------------------------------
| Check If The Application Is Under Maintenance
|--------------------------------------------------------------------------
|
| If the application is in maintenance / demo mode via the "down" command
| we will load this file so that any pre-rendered content can be shown
| instead of starting the framework, which could cause an exception.
|
*/

// Check for maintenance mode
$maintenancePath = __DIR__.'/../storage/framework/maintenance.php';
if (!file_exists($maintenancePath)) {
    $maintenancePath = dirname(__DIR__).'/storage/framework/maintenance.php';
}
if (file_exists($maintenancePath)) {
    require $maintenancePath;
}

/*
|--------------------------------------------------------------------------
| Register The Auto Loader
|--------------------------------------------------------------------------
|
| Composer provides a convenient, automatically generated class loader for
| this application. We just need to utilize it! We'll simply require it
| into the script here so we don't need to manually load our classes.
|
*/

// For cPanel shared hosting, ensure correct path resolution
$autoloadPath = __DIR__.'/../vendor/autoload.php';
if (!file_exists($autoloadPath)) {
    // Try alternative path for some hosting configurations
    $autoloadPath = dirname(__DIR__).'/vendor/autoload.php';
}
require $autoloadPath;

/*
|--------------------------------------------------------------------------
| Run The Application
|--------------------------------------------------------------------------
|
| Once we have the application, we can handle the incoming request using
| the application's HTTP kernel. Then, we will send the response back
| to this client's browser, allowing them to enjoy our application.
|
*/

// Load Laravel application
$appPath = __DIR__.'/../bootstrap/app.php';
if (!file_exists($appPath)) {
    $appPath = dirname(__DIR__).'/bootstrap/app.php';
}
$app = require_once $appPath;

$kernel = $app->make(Kernel::class);

$response = $kernel->handle(
    $request = Request::capture()
)->send();

$kernel->terminate($request, $response);
