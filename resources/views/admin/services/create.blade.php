...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-gray-600 mt-1">Add a new service</p>
        </div>
        ...
        <form action="{{ route('admin.services.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="meta" name="meta" rows="2" class="admin-input @error('meta') border-red-500 @enderror">{{ old('meta') }}</textarea>
                    @error('meta')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="md:col-span-2">
                    <label class="flex items-center">
                        <input type="checkbox" name="home" value="1" {{ old('home') ? 'checked' : '' }} 
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="content" name="content" rows="8" required class="admin-input @error('content') border-red-500 @enderror">{{ old('content') }}</textarea>
                    @error('content')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="content_extra" name="content_extra" rows="6" class="admin-input @error('content_extra') border-red-500 @enderror">{{ old('content_extra') }}</textarea>
                    @error('content_extra')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
